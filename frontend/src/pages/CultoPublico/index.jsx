...
function EstudarVisitante({ musicas, indiceInicial = 0, nomeCulto, onFechar }) {
  ...
  // ── Novo: Autoplay (mesmo padrão já estabilizado no Estudar interno) ──
  const [autoplay, setAutoplay] = useState(() => {
    try { return localStorage.getItem('lcv_autoplay_visitante') === '1'; } catch { return false; }
  });
  ...
  function next() { if (temProxima) setIdx(i => i + 1); }
  function prev() { if (temAnterior) setIdx(i => i - 1); }
  ...
  useEffect(() => {
    try { localStorage.setItem('lcv_autoplay_visitante', autoplay ? '1' : '0'); } catch {}
  }, [autoplay]);

  // ── Novo: música sem vídeo + Autoplay ligado → avança sozinho após alguns segundos ──
  useEffect(() => {
    ...
    >
      ...
      <div className={styles.palcoNav}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '16px 24px 36px', overflow: 'hidden' }}>
          ...
          {musica?.artista && (
            <div style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginBottom: 28 }}>{musica.artista}</div>
          )}

          <div style={{ display: 'flex', alignItems: 'center', gap: 26, marginBottom: 22 }}>
            <button
              onClick={prev} disabled={!temAnterior}
              style={{
                background: 'transparent', border: 'none', fontSize: 28, padding: 6,
                color: !temAnterior ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.6)',
                cursor: !temAnterior ? 'default' : 'pointer', fontFamily: 'inherit',
              }}
            >⏮</button>
            ...
            <button
              onClick={next} disabled={!temProxima}
              style={{
                background: 'transparent', border: 'none', fontSize: 28, padding: 6,
                color: !temProxima ? 'rgba(255,255,255,0.15)' : 'rgba(255,255,255,0.6)',
                cursor: !temProxima ? 'default' : 'pointer', fontFamily: 'inherit',
              }}
            >⏭</button>
          </div>
