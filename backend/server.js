...
app.get('/public/culto/:token', async (req, res) => {
  try {
    ...
    const repertorioResult = await pool.query(`
      SELECT ri.id, ri.posicao, ri.tipo, ri.descricao, ri.tom_culto, ri.louvor_id, ri.vocal_principal_id,
    ...
    const vocalistasIds     = escalaResult.rows
      ...
      .map(e => e.membro_id);
    const tonsPorLouvor     = {}; // louvor_id -> [{ membro_id, tom, vocal }]

    if (vocalistasIds.length > 0) {
      const tomsResult = await pool.query(`
        SELECT ltv.louvor_id, ltv.membro_id, ltv.tom, m.nome AS vocalista_nome
      ...
      tomsResult.rows.forEach(row => {
        if (!tonsPorLouvor[row.louvor_id]) tonsPorLouvor[row.louvor_id] = [];
        tonsPorLouvor[row.louvor_id].push({ membro_id: row.membro_id, tom: row.tom, vocal: row.vocalista_nome });
      });
    }

    // ── Montar resposta ───────────────────────────────────────────────────
    const repertorio = repertorioResult.rows.map(item => {
      const candidatos = item.louvor_id ? (tonsPorLouvor[item.louvor_id] || []) : [];
      // Resolve: vocal principal marcado > único vocalista escalado com tom salvo > nenhum (ambíguo)
      let resolvido = item.vocal_principal_id
        ? candidatos.find(c => c.membro_id === item.vocal_principal_id) || null
        : null;
      if (!resolvido && candidatos.length === 1) resolvido = candidatos[0];
      ...
      return {
        ...
        letra:       item.letra,
        // Tom: prioridade tom_culto (ajuste manual) → tom do vocal principal/único escalado → tom base do louvor
        tom:         item.tom_culto || resolvido?.tom || item.tom_padrao || null,
        vocal:       resolvido?.vocal || null,
      };
    });
