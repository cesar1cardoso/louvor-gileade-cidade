...
router.patch('/:id/itens/:itemId', liderOuAdminMiddleware, async (req, res) => {
  const { tom_culto, duracao_min, observacao, descricao, vocal_principal_id } = req.body;
  try {
    const result = await pool.query(
      `UPDATE repertorio_itens SET
        ...
        vocal_principal_id = COALESCE($5, vocal_principal_id)
       WHERE id = $6 AND repertorio_id = $7 RETURNING *`,
      [tom_culto, duracao_min, observacao, descricao, vocal_principal_id, req.params.itemId, req.params.id]
    );
